  ...
  try {
    ...
    for (let i = 0; i < recipes.length; i++) {
      ...
      if (recipe.ustensiles) {
        for (let k = 0; k < recipe.ustensiles.length; k++) {
          let ustensil = recipe.ustensiles[k];
    ...
    //Appliances list
    for (let i = 0; i < sortedAppareils.length; i++) {
      let item = sortedAppareils[i];
      const li = factory.createListItem(item);
      li.addEventListener('click', function (event) {
        // Create new element
        const newElement = document.createElement('div');
    ...
    //Ustensils list
    for (let i = 0; i < sortedUstensiles.length; i++) {
      let item = sortedUstensiles[i];
      console.log('Adding ustensil:', item); // Add this line
      const li = factory.createListItem(item);
      li.addEventListener('click', function (event) {
        // Create new element
        const newElement = document.createElement('div');
        ...
        console.log('Adding element to filter section:', filterSection); // Add this line
        filterSection.appendChild(newElement);
        // Filter function 
        filter(event);
      });
...
/**
 * Self-invoking anonymous function that fetches the recipe data and populates the dropdowns.
 * @async
 * @function
 */
(async () => {
